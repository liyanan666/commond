...
<script>
	...
	Vue.filter('showkeyword', function(value,Keyword) {
		console.log(Keyword);
		if(Keyword == ''){
			return value;
		}else if (!value) { 
		  ...
		  var result = value.replace(reg,"<span style='color:#F00;font-weight:700;'>"+Keyword+"</span>");
		  return result;
//		return '123';
	});
	export default {
...
<style>
	...
	.commond-from,.conmond-title,.conmond-addr{
		line-height: 35px;
	}
	.conmond-title{
		flex: 1;
	...
	.el-pager li.active+li,.el-pagination .btn-next,.el-pagination .btn-prev{
		border: 1px solid #d1dbe5;
	}
